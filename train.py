...
class EMGIMUGUI(QMainWindow):
    ...
    def save_model(self):
        ...
        if hasattr(self, 'model') and self.model is not None:
            file_path, _ = QFileDialog.getSaveFileName(self, "Save Model", "", "PyTorch Model (*.pt *.pth)")
            if file_path:
                try:
                    ...
                    clean_results = {
                        ...
                        "predictions": self.test_results["predictions"].tolist(),
                        "true_labels": self.test_results["true_labels"].tolist(),
                        "report": str(self.test_results["report"])
                    }
                    ...
                    self.status.setText(f"Model saved to: {file_path}")
                except Exception as e:
                    self.status.setText(f"Error saving model: {str(e)}")
                    
    def load_model(self):
        try:
            file_path, _ = QFileDialog.getOpenFileName(self, "Load Model", "", "Model Files (*.pt *.pth)")
            if not file_path:
                return

            import numpy as np
            import torch
            torch.serialization.add_safe_globals({'numpy.core.multiarray._reconstruct': np.core.multiarray._reconstruct})
            ...
            sequence_length = checkpoint.get("sequence_length", 20)
            model_type = checkpoint.get("model_type", "LSTM")
            layer_types = checkpoint.get("layer_types", [model_type])

            # Déduire s’il s’agit d’un modèle Hybrid ou pas
            if isinstance(layer_types, list) and len(set(layer_types)) > 1:
                model_to_load = layer_types  # Hybrid
            else:
                model_to_load = layer_types[0] if isinstance(layer_types, list) else layer_types

            # Construire le modèle
            model = get_model(model_to_load, input_size, num_classes, num_layers, sequence_length)
            ...
            if test_results:
                ...
                test_results["true_labels"] = np.array(test_results["true_labels"])

            # Stocker dans l'interface
            self.model = model
            self.history = history
            self.test_results = test_results

            if history and test_results:
                self.plot_results(history, test_results, optimizer=None)

            self.status.setText(f"Model loaded from: {file_path}")

        except Exception as e:
            QMessageBox.warning(self, "Load Error", f"Erreur lors du chargement du modèle :\n{str(e)}")
